...
interface IV2SwapRouter {
    ...
    function exactInputSingle(SushiswapParams calldata params)
        external
        payable
        returns (uint256 amountOut);
}
...
contract BalancerFlashloan is IFlashLoanRecipient {
    ...
    // IV2SwapRouter public sushiRouter;
    // IV2SwapRouter public quickRouter;
    ...
    address public balancerAddress = 0xBA12222222228d8Ba445958a75a0704d566BF2C8;
    ...
    constructor(
        ...
        // IV2SwapRouter _sushiRouter,
        // IV2SwapRouter _quickRouter
        address _swapRouter,
        ...
        owner = msg.sender;
        // mimatic = ERC20(mimaticAddress);
    }

    function makeArbitrage(address tokenIn, address tokenOut) public {
        uint256 amountIn = IERC20(tokenIn).balanceOf(address(this));
        Exchange result = _comparePrice(tokenIn, tokenOut, amountIn);
        if (result == Exchange.UNI) {
            // sell ETH in uniswap for DAI with high price and buy ETH from sushiswap with lower price
            uint256 amountOut = _swap(amountIn, swapRouter, tokenIn, tokenOut);
            _swap(amountOut, sushiRouter, tokenOut, tokenIn);
        } else if (result == Exchange.SUSHI) {
            // sell ETH in sushiswap for DAI with high price and buy ETH from uniswap with lower price
            uint256 amountOut = _swap(amountIn, sushiRouter, tokenIn, tokenOut);
            _swap(amountOut, swapRouter, tokenOut, tokenIn);
        }
    }
    ...
    function _comparePrice(
        ...
        address tokenOut,
        uint256 amount
    ) internal view returns (Exchange) {
        uint256 uniswapPrice = _getPrice(swapRouter, tokenIn, tokenOut, amount);
    ...
    */
    function receiveFlashLoan(
        ...
        (address token0, address token1) = abi.decode(userData, (address, address));
        // address token0Address = address(tokens[0]);
        // address token1Address = address(tokens[1]);
        makeArbitrage(token0, token1);
    ...
    function executeFlashLoan(
        ...
        //we are borrowing token from Balancer
        IERC20[] memory assets = new IERC20[](1);
        // assets[0] = mimatic;
        assets[0] = IERC20(token0);

        // the amount to be flashed for each asset
        uint256[] memory amounts = new uint256[](1);
        amounts[0] = _amount;

        balancer.flashLoan(this, assets, amounts, abi.encode(token0, token1));
    }
    ...
    function setQuickRouter(address _quickRouter) external {
        quickRouter = _quickRouter;
    }
    ...
    function setBalancerAddress(address _balancerAddress) external onlyOwner {
        balancerAddress = _balancerAddress;
        balancer = Balancer(_balancerAddress);
    }
