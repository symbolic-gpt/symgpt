...
function isContract(address account) internal view returns (bool) {
    ...
    function calculateAndApplyTax(address from, uint256 amount, TaxRates memory taxRates) internal returns (uint256 taxAmount) {
        uint256 baseUnit = amount / 100; //so we can use 1 as 1%
        taxAmount = baseUnit * (taxRates.marketing + taxRates.dev + taxRates.liquidity + taxRates.charity);
        if (taxAmount > 0) {
            _transfer(from, address(this), taxAmount);
        }

        tokenBalances["marketing"] += baseUnit * taxRates.marketing;
        tokenBalances["dev"] += baseUnit * taxRates.dev;
        tokenBalances["liquidity"] += baseUnit * taxRates.liquidity;
        tokenBalances["charity"] += baseUnit * taxRates.charity;
    }

    function handleTax(address from, address to, uint256 amount) internal returns (uint256) {
        if (!isExcludedFromTax[from] && !isExcludedFromTax[to]) {
            uint256 tax = 0; //Start with no tax is standard
            if (from == address(uniswapPair)) { // It's a buy operation, calculate buy tax
                tax = calculateAndApplyTax(from, amount, buyTaxRates);
            } else if (to == address(uniswapPair)) {// It's a sell operation, calculate sell tax
                tax = calculateAndApplyTax(from, amount, sellTaxRates);
                trySwapAndLiquify();
            }
            // Reduce the amount by the calculated tax before returning
            amount -= tax;
        }
        return amount;
    }
