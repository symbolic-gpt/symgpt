...
interface IERC20Errors {
    ...
    /**
     * @dev Indicates a failure with the `spender`ظآs `allowance`. Used in transfers.
...
interface IERC721Errors {
    ...
    /**
     * @dev Indicates a failure with the `operator`ظآs approval. Used in transfers.
...
library ECDSA {
    ...
    function tryRecover(
        ...
        // the valid range for s in (301): 0 < s < secp256k1n ├╖ 2 + 1, and for v in (302): v ظêê {27, 28}. Most
...
library Math {
    ...
    function sqrt(uint256 a) internal pure returns (uint256) {
        ...
        // ظْ `sqrt(2**k) <= sqrt(a) < sqrt(2**(k+1))`
        // ظْ `2**(k/2) <= sqrt(a) < 2**((k+1)/2) <= 2**(k/2 + 1)`
        //
...
pragma solidity ^0.8.20;
contract Uscoin is ERC20, ERC20Burnable, ERC20Pausable, Ownable, ERC20Permit {
    constructor(address initialOwner)
        ERC20("uscoin", "USCO")
