...
contract SCPNFTs is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public pricePresale = 0.3 ether;
  ...
  // Lists 
  uint256[502] public remainingIds;
  ...
  // Constructor
  constructor() ERC721("Pigz & Robbers", "PIG")    {
    }
  ...
  function setlastSupply(uint256 _lastSupply) public onlyOwner {
  ...
  function mint(address to, uint256 _mintAmount) external payable nonReentrant {
    ...
    require(_mintAmount <= maxMintAmount,  "You can only mint 15 at a time");
  ...
  function _getRandom() internal view returns (uint256) {
    return
      uint256(
        keccak256(
          abi.encodePacked(blockhash(block.number-1), block.timestamp, lastSupply)
        )
      );
  }
}