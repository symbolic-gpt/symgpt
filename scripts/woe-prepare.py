...
{{
    ...
    for result, erc_file in zip(results, erc_files):
        with open(erc_file + "_out", "w") as f:
            json.dump(extract_constraints(result.choices[0].message.content), f, indent=2)
...
def extract_constraints(content):
    """Normalize the JSON-object response into the requested rule array."""
    ...
    raise ValueError("Expected a JSON array or an object containing exactly one JSON array")
