...
contract INTDBridge is Ownable {
    ...
    event Swapped(address indexed swapper, uint256 amount, uint256 targetChain);
    ...
    function setWhitelist(address[] calldata _whitelist) external onlyOwner {
        require(
            _whitelist.length > 0,
            "Bridge: _whitelist should not be empty"
        );
    ...
    function deposit(uint256 _amount, uint256 _targetChain) external {
        require(isWhitelist[msg.sender], "Bridge: not in whitelist");
        ERC20Burnable(token).burnFrom(msg.sender, _amount);

        swappedAmount[msg.sender] += _amount;

        if (!isSwapped[msg.sender]) {
            totalSwapers += 1;
            swappers.push(msg.sender);
            isSwapped[msg.sender] = true;
        }

        emit Swapped(msg.sender, _amount, _targetChain);
    }

    function swappersDetailsList(
        uint _from,
        uint _to
    ) external view returns (SwaperDetails[] memory) {
        require(_from < _to, "Bridge: _from should be less than _to");
        ...
        for (uint i = from; i < to; i += 1) {
            amount[i] = SwaperDetails(swappers[i], swappedAmount[swappers[i]]);
        }
