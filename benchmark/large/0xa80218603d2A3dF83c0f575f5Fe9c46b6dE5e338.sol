...
contract PigzAndRobbers is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      // Split payment
      uint256 mintShareM = _amount.div(mintReward); //7% CosmicNomics
      uint256 mintShareS = _amount.div(mintReward); //7% SilentInvestor
      ...
      payable(0xd294d8E5d0E4A0BCc8fb13ed7337132da52fe833).transfer(mintShareS); // CosmicNomics
      payable(0xB2d4ECb7DD0989974A0A0183bdca560b4741f4a2).transfer(mintShareM); // StakeVault(atm)
