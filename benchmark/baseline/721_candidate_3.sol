...
contract AffiliateProgram is Pausable {
    ...
    payable
  {
    ...
    lastDepositTimes[_affiliate] = block.timestamp; // solium-disable-line security/no-block-members
    lastDepositTime = block.timestamp; // solium-disable-line security/no-block-members
   ...
   */
  function _performWithdraw(address _from, address payable _to) private {
  ...
  function withdraw() public whenNotPaused {
    _performWithdraw(msg.sender, payable(msg.sender));
  }
  ...
  function withdrawFrom(address _affiliate, address _to) onlyOwner public {
    ...
    require(block.timestamp > lastDepositTimes[_affiliate].add(commissionExpiryTime));
    _performWithdraw(_affiliate,payable(_to));
  }
   ...
   */
  function retire(address payable _to) onlyOwner whenPaused public {
    // solium-disable-next-line security/no-block-members
    require(block.timestamp > lastDepositTime.add(commissionExpiryTime));
   ...
   */
  function unpause() override onlyOwner whenPaused public {
    require(!retired);
    paused = false;
    emit Unpause();
  }
...
abstract contract ERC721 {
  ...
  function setApprovalForAll(address _to, bool _approved) external virtual;
...
contract LicenseOwnership is LicenseInventory, ERC721, ERC165, ERC721Metadata, ERC721Enumerable {
  ...
  function setTokenMetadataBaseURI(string memory _newBaseURI) external onlyCEOOrCOO {
...
contract LicenseSale is LicenseOwnership {
   ...
   */
  uint256 public renewalsCreditAffiliatesFor = 365 * 24 * 60 * 60;
    ...
    returns (uint)
  {
    ...
    uint256 expirationTime = isSubscriptionProduct(_productId) ?
      SafeMath.add(block.timestamp, SafeMath.mul(intervalOf(_productId), _numCycles)) : // solium-disable-line security/no-block-members
      0;

    License memory _license = License({
      productId: _productId,
      attributes: _attributes,
      issuedTime: block.timestamp, // solium-disable-line security/no-block-members
      expirationTime: expirationTime,
      affiliate: _affiliate
    });

    licenses.push(_license);
    uint256 newLicenseId = licenses.length - 1; // solium-disable-line zeppelin/no-arithmetic-operations
    emit LicenseIssued(
      _assignee,
      msg.sender,
      newLicenseId,
  ...
  function _performRenewal(uint256 _tokenId, uint256 _numCycles) internal {
    ...
    uint256 renewalBaseTime = Math.max256(block.timestamp, licenses[_tokenId].expirationTime);

    // We assume that the payment has been validated outside of this function
    uint256 newExpirationTime = SafeMath.add(renewalBaseTime, SafeMath.mul(intervalOf(productId), _numCycles));
  ...
  */
    ...
    returns (uint256)
  {
    ...
    uint256 attributes = uint256(keccak256(abi.encodePacked(blockhash(block.number-1), _productId, _assignee)));
    ...
    whenNotPaused
  {
    ...
    if(
      ...
      _affiliateProgramIsActive() &&
      licenseIssuedTime(_tokenId) + renewalsCreditAffiliatesFor > block.timestamp
    ) {
      _handleAffiliate(
