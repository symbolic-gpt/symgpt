...
contract CosmoonautsDGR_v1 is VRFConsumerBaseV2, ReentrancyGuard, AutomationCompatibleInterface {
...
//Constructor
    ...
    constructor(
        ...
        bytes32 _hashCle,
        FGToken _FGT
    ) VRFConsumerBaseV2(_coordonnateurVRF) {
        coordonnateurVRF = _coordonnateurVRF;
        hashCle = _hashCle;
        COORDINATEUR = VRFCoordinatorV2Interface(coordonnateurVRF);
    ...
    // These parameters set the crew size to 20, the grand prize to 0.08 MATIC, 
    // 2 secondary prizes of 0.032 MATIC each, 3 tertiary prizes of 0.024 MATIC each, 4 fourth prizes of 0.022 MATIC.
...
//VRF Rocket Science Section
    ...
    function demanderNombresAleatoires(uint256 nbMots) internal {
        ...
        for (uint256 i = 0; i < nombreRequetes; i++) {
            ...
            uint256 requestId = COORDINATEUR.requestRandomWords(
                hashCle,
                abonnementId,
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory motsAleatoires)
        internal
        override
    {
