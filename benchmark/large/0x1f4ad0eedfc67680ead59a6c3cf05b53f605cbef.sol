...
contract IlluminatedEdgePunks is ERC721A, Ownable, ReentrancyGuard {
  
    bool public presaleActive;
    bool public publicsaleActive;
    uint256 public DmiPerTransaction = 3;
    uint256 public constant SUPPLY = 9997;
    string public baseURI;
    mapping (address => uint256) public presaleClaimAmount;

    constructor(address initialOwner) Ownable(initialOwner) ERC721A("Illuminated EdgePunks", "IEP") {}
    ...
    function setDmiPerTransaction(uint256 count) external onlyOwner {
        DmiPerTransaction = count;
    }
    ...
    function presaleMint(uint256 count) payable public {
        ...
        if (SUPPLY < (totalSupply() + count)) revert SoldOut();
        if (count > DmiPerTransaction) revert MaxPerTransactionExceeded();
        if (presaleClaimAmount[msg.sender] + count > DmiPerTransaction) revert AlreadyMintedDuringPresale();
