...
// File: contracts/ZbyteContext.sol
...
// File: contracts/TheFWordBatchMinter.sol
...
contract TheFWordBatchMinter is ERC721, ERC721URIStorage, ZbyteContext {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    uint256 startTokenId = 0;

    event BatchMintTransfer(address indexed _from, address indexed _to, uint256 startTokenId, uint256 endTokenId);
    event SafeTransfer(address indexed _from, address indexed _to, uint256 tokenId);

    constructor() ERC721("TheFWordBatchMinter", "TFWBM") {}
    ...
    function safeMint(address to, string memory uri) public onlyOwner {
        ...
        emit SafeTransfer(address(this), to, tokenId);
    }
    ...
    function batchMint(address to, string[] memory metadataUri, uint256 batchSize) public onlyOwner {
        _tokenIdCounter.increment();
        startTokenId = _tokenIdCounter.current();
        _safeMint(to, startTokenId);
        _setTokenURI(startTokenId, metadataUri[0]);
        for (uint256 i = 1; i < batchSize; i++) {
            _tokenIdCounter.increment();
            uint256 tokenId = _tokenIdCounter.current();
            _safeMint(to, tokenId);
            _setTokenURI(tokenId, metadataUri[i]);
        }
        emit BatchMintTransfer(address(this), to, startTokenId, _tokenIdCounter.current());
    }

    function getBatchMintedTokenIds() public view returns (uint256, uint256) {
        return (startTokenId, _tokenIdCounter.current());
    }
