...
class SlicedLLMSolAuditor(LLMERCAuditor):
    ...
    def process(self, sol_file:str, output_dir:str, **kwargs):
        ...
        for contract in ctx.metadata.contracts:
            with open(os.path.join(self._erc_dir, f"build/ERC{erc}_ERC{erc}.json"), "r") as f:
                erc_obj = json.load(f)
                ...
                try:
                    ...
                    if obj["def"].startswith("event"):
                        if rtype == "interface":
                            ...
                            prompt = f"""By given the following solidity event interfaces:\"\"\"\n{event_interfaces}\n\"\"\"\nCheck if the code contains the interface "{rule}", return in "YES" or "NO"."""
...
{fstr}
\"\"\"
Check if the code violated the rule "{rtype} {rule} {("if "+cond) if cond else ""}", return in "YES" or "NO".
"""                         
                                cnt += 1
                                res = self._llm.single(prompt, temperature=0, n=1)[0]
