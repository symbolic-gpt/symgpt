...
// File @openzeppelin/contracts/token/ERC721/extensions/ERC721Royalty.sol@v5.0.2
...
abstract contract CustomERC721 is ERC721, ERC721URIStorage, ERC721Burnable, ERC721Royalty, ERC721Enumerable, Ownable {
...
// File hardhat/console.sol@v2.22.5
...
// File contracts/Lumissoly_V1.sol
...
contract Item_V1 is CustomERC721 {
    uint256 private _nextTokenId;
    string private baseItemUri;

    // Mapping to store UUIDs that have already been minted
    mapping(string => bool) private _mintedUUIDs;

    /**
     * @dev Emitted when the token is mint.
     */
    event Minted(address indexed to, string skuId, address royaltiesReceiver, uint8 royaltiesFees, string uuid, uint256 tokenId);

    constructor(
        string memory _baseItemUri
    ) CustomERC721(_msgSender(), "Eclat by Lumissoly", "OBJECTIFY#1") {
    ...
    /**
     * @dev The denominator with which to interpret the fee set in {_setTokenRoyalty} and {_setDefaultRoyalty} as a
     * fraction of the sale price. Defaults to 100 so fees are expressed in percent points.
     */
    function _feeDenominator() internal pure override returns (uint96) {
        return 100;
    }

    function safeMint(address to, string memory skuId, address royaltiesReceiver, uint8 royaltiesFees, string memory uuid) public onlyOwner {
        // Check and mark the UUID as minted
        require(!_mintedUUIDs[uuid], "UUID has already been minted");
        _mintedUUIDs[uuid] = true;

        // Generate the next tokenId
        uint256 tokenId = _nextTokenId++;

        // Standard mint
        _safeMint(to, tokenId);

        // Set token royalties infos
        _setTokenRoyalty(tokenId, royaltiesReceiver, royaltiesFees);

        // Set the dynamic NFT metadata
        _setTokenURI(tokenId, uuid);

        // Notify the off-chain engine to process the mint post processing actions
        emit Minted(to, skuId, royaltiesReceiver, royaltiesFees, uuid, tokenId);

        // Notify the external marketplace to fetch metadata for this tokenId
        emit MetadataUpdate(tokenId);
    }
