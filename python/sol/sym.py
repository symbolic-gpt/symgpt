...
from z3 import *
from sol.utils import compile, emit_if_vars_changed, get_anchored_state_variable, get_the_function
...
from slither.slithir.variables import Constant, TemporaryVariable, TupleVariable
from slither.core.solidity_types import ElementaryType, MappingType, ArrayType
from slither.core.declarations import Contract, FunctionContract, SolidityFunction, SolidityVariableComposed
...
def deserialize_verify(json_data: str | dict) -> TypedBase:
    ...
    if isinstance(json_data, dict):
        ...
        if data.get('type') in type_map:
            return type_map[data['type']].from_dict(json_data)
        else:
        ...
        if data.get('type') in type_map:
            return type_map[data['type']].from_json(json_data)
        else:
            raise ValueError("Unknown verify type")
...
def get_var_from_selector(selector: FuncParamSelector | ConstantSelector | StateVarSelector | MsgSenderSelector | EventParamSelector | FnCallParamSelector | NoneValueSelector | FnCallRetSelector, 
    ...
    if isinstance(selector, FuncParamSelector):
        ...
        if fn_call is None:
            raise Exception(f"fn_call is None")
            ...
            for key in selector.keys:
                if key == "msg.sender" or isinstance(key, MsgSenderSelector):
                    ...
                    if isinstance(key_z3, ArrayRef):
                        print(f"key_z3 {key_z3}, use_index_at_arr {use_index_at_arr}")
...
class CompilationUnitNotSet(Exception):
    def __init__(self) -> None:
        super().__init__(f"no compilation unit is set")
...
class ErcVerifier:
    ...
    def run(self, contract_name:str, function_name:str, fnumofargs:int, vop, fn:FunctionContract=None) -> bool:
        ...
        print(f"found array parameters: {len(to_exec) > 1}")
        for i, init_exec in enumerate(to_exec):
            curr_arr_len = None
            if len(to_exec) > 1:
                curr_arr_len = forks_at_array_lens[i]
                print(f"fork at array length {curr_arr_len}")
            ...
            if isinstance(vop, ThrowVerify):
                ...
                if vop.sv_cond is not None:
                    ...
                    if vop.sv_cond.change_type is not None:
                        ...
                        if len(sv_types) < 2:
                            raise Exception(f"balance types should be mapping")
        ...
        while to_exec:
            ...
            try:
                ...
                if len(nexts) == 0:
                    ...
                    selected_exec.solver.add(Bool(f"#throwed") == False)
                elif len(nexts) == 1:
                    ...
                    if throwed:
                        ...
                        selected_exec.solver.add(Bool(f"#throwed") == True)
                    ...
                    for next, curr_exec in zip(nexts, forked_execs):
                        ...
                        if throwed:
                            ...
                            curr_exec.solver.append(Bool(f"#throwed") == True)
                        else:
                            curr_exec.solver.append(*next_constraints)
        ...
        if isinstance(vop, ThrowVerify):
            ...
            for exec in self.execs:
                ...
                print(f"state assign buggy z3: {buggy_and}")
