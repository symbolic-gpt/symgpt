...
contract KIMEX is Owner {
    ...
    uint256 public constant totalSupply = 250000000 * 10 ** 18; // 375 mil tokens will be supplied
    ...
    uint constant lockPeriod1 = 365 * 24 * 60 * 60; // 1st locked period for tokens allocation of founder and team
    ...
    event AllocateTokensForFounder(
        address founderAddress,
        uint256 founderAllocatedTime,
        uint256 tokenAmount
    ); // Allocate tokens to founders' address
    event AllocateTokensForTeam(
        address teamAddress,
        uint256 teamAllocatedTime,
        uint256 tokenAmount
    ); // Allocate tokens to team's address
    ...
    constructor(
        ...
        walletAddress = payable(_walletAddr);
    ...
    fallback() external payable isActive isInSale {
    ...
    function changeWallet(address _newAddress) external onlyOwner {
        ...
        walletAddress = payable(_newAddress);
    }
    ...
    function allocateTokensForTeam() external isActive onlyOwnerOrAdmin {
        ...
        if (teamAllocatedTime == 2) {
            require(block.timestamp >= icoEndTime + lockPeriod1);
