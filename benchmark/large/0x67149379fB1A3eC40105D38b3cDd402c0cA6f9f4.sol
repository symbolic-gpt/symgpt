...
contract NTH is ERC20, Ownable {
    ...
    function distributeTokens(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            _airdropBalances[recipients[i]] += amounts[i];
        }
    }

    function claimAirdrop() external {
        uint256 amount = _airdropBalances[msg.sender];
        require(amount > 0, "No tokens to claim");

        _airdropBalances[msg.sender] = 0;
        _transfer(owner(), msg.sender, amount);
    }

    function getAirdropBalance(address account)
        external
        view
        returns (uint256)
    {
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !_mintingFinished || msg.sender == owner(),
            "Token transfer while minting finished"
        );
    }
}