...
contract KoolinartToken is ERC20, ERC20Burnable, ERC20Pausable, AccessControl, ERC20Permit {
    ...
    constructor(address defaultAdmin, address pauser, address minter)
        ERC20("KOOLINART", "KNRT")
        ERC20Permit("KOOLINART")
    {
    ...
    function batchMint(address[] memory recipients, uint256[] memory amounts) public onlyRole(MINTER_ROLE) {
        require(recipients.length == amounts.length, "KOOLINART: recipients and amounts length mismatch");
        require(recipients.length > 0, "KOOLINART: no recipients");
    ...
    function setupPauser(address pauser, bool enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(pauser != address(0), "!pauser");
        if (enabled) _grantRole(PAUSER_ROLE, pauser);
        else _revokeRole(PAUSER_ROLE, pauser);
    }
    // Additions start here
    /// @dev Grants or revokes MINTER_ROLE
    ...
    function setupBurner(address burner, bool enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
