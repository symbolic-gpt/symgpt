...
contract SPOZZ is ERC20, ERC20Burnable, Ownable {
    uint256 private immutable i_MaximumSupply;
    constructor(address initialOwner)
        ERC20("SPOZZ", "SPZZ")
        Ownable(initialOwner)
    {
        i_MaximumSupply = 10000000000 * 10 ** decimals();
        _mint(initialOwner, 100000000 * 10 ** decimals());
    }

    function mint(address to, uint256 amount) public onlyOwner {
        require(totalSupply() + amount <= i_MaximumSupply, "Cannot mint more than the maximum supply.");
        _mint(to, amount);
    }

    function getMaximumSupply() public view returns(uint256){
        return i_MaximumSupply;
    }
}