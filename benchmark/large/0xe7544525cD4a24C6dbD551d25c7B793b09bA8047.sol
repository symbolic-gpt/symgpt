...
contract ExqDaoErc1155Collection is ERC1155Burnable, AccessControl {
    ...
    // Maps the NFT Id to the TierNFT
    mapping(uint256 => TierNFT) public TierNFTs; // Maps NFTId by TierNFTStruct
    // NFT id -> NFT price
    ...
    constructor() ERC1155("https://api.fantomstarter.io/nft/") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

        TierNFTs[1] = TierNFT(true, 1, 5000, "ROCKETEER");
    }
    ...
    function uri(
        ...
        require(TierNFTs[_id].initialized == true, "NFT does not exist");
     ...
     **/
    function getTierByNFTId(
        uint256 _nftId
    ) public view returns (uint256 _tier) {
        return TierNFTs[_nftId].tier;
    }

    // Checks
    /**
     * @dev Checks if the token can be minted.
     * returns boolean
     **/
    function isMintable(uint256 _nftId) public view returns (bool) {
        require(TierNFTs[_nftId].initialized == true, "NFT does not exist");
        return totalSupply(_nftId) < TierNFTs[_nftId].maxSupply; // Is mintable only if the current supply did not exceed the max supply
    }

    // BusinessLogic
    /**
     * @dev Add a new Tier to the Factory, you can not add a Tier with ID=0
     * Only Admins are allowed to add Tiers
     **/
    function createTierNFT(
        uint256 _nftId,
        uint256 _tier,
        uint256 _maxSupply,
        string memory _name
    ) public isAdmin {
        require(_nftId != 0, "You cannot use the default 0 as id for tier");
        require(
            TierNFTs[_nftId].initialized == false,
            "NFT already exists for given ID"
        );
        TierNFTs[_nftId] = TierNFT(true, _tier, _maxSupply, _name);
    }
    ...
    function mint(
        ...
        require(isMintable(_nftId) == true, "NFT no longer mintable");
        _mint(_receiver, _nftId, _quantity, _data);
        // Mints 1 token to the receiver
    }

    function buyNFTWithNativeToken(uint256 _nftId) external payable {
        require(
            isMintable(_nftId) == true,
            "This NFT is can not be bought at this time"
        );
        uint256 _nftPrice = nftPrice[_nftId];
        require(msg.value == _nftPrice, "Not enough native token sent!");
    ...
    function withdraw() external isAdmin {
        require(
            ...
            "Contract doesn't have any balance!"
        );
