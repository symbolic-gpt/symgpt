...
contract ERC1155NFTCustom is ERC1155, AccessControl {
    ...
    constructor(string memory _name, string memory _symbol, address _contractowner, bool _metadataUpdatable, bool _tokensBurnable, bool _tokensTransferable, string memory _initBaseURI,  address _royaltiesAddress, uint96 _royaltiesBasisPoints) {
    ...
    function mintByOwner( address account, uint256 id, uint256 amount, string memory minturi) public onlyRole(MINTER_ROLE) {
        require(!_exists(id), "NFT: token already minted");
        if (bytes(minturi).length > 0) {
            _tokenURIs[id] = minturi;
            emit URI(minturi, id);
        }
