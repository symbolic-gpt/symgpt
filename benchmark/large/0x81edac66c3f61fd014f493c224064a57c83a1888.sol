...
contract CLAIM is ERC721, Ownable {
    string public baseuri = "ipfs.io/ipfs/QmRKjFgp8edoNai7p7gH7L3jQRWBfwYfaxWYJPSt8ejLaQ";
    uint public _tTotal;
    ...
    function safeMint(address to) public onlyOwner {
        _safeMint(to, _tTotal);
        _tTotal++;
    }

    function massNFT(address[] memory addresses) external onlyOwner {
        uint count = addresses.length;
        for (uint i; i < count; i++) {
            _safeMint(addresses[i], _tTotal);
            _tTotal++;
        }
    }
}