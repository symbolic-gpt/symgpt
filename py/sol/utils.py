...
from slither.slithir.operations import InternalCall, LibraryCall, EventCall, Operation, HighLevelCall, Assignment, Index
    ...
    if args:
        for arg in args.split(','):
            ...
            if len(arg_parts) >= 2:
                ...
                arg_type = ' '.join([part for part in arg_parts[:-1] if part != "calldata" or part != "memory"] )
...
def parse_event_declaration(declaration: str) -> SolidityEventFormat:
    ...
    if not match:
        raise ValueError(f"Invalid event declaration: '{declaration}'")
...
def compile(
    ...
    try:
        ...
        # if version is 0.8.0 and cannot be compiled, usually can be compiled by 0.8.20
        # this is due to many contract with ^0.8.0, but actually they are compiled by 
        # compiler version above 0.8.0
        if solc_version == "0.8.0":
            ensure_solc_version(None, "0.8.20")
            slither = Slither(sol_file_or_dir)
            cu = slither.compilation_units[0]
            return "0.8.20", cu
        raise ex
...
def get_the_function(cu: SlitherCompilationUnit, contract_name:str, fi: SolidityFunctionFormat=None, fname:str = None, fnumofargs:int=None) -> Union[FunctionContract, StateVariable]:
    ...
    if fnumofargs is None or fnumofargs == 0:
        # check whehther the function name has corresponding state variable
        for sv in c.state_variables_ordered:
            if sv.name == function_name:
                return sv
...
# def get_anchor_functions(erc:str) -> List[str]:
#     erc_json = get_erc(erc)
#     if erc_json is None:
#         return None
#     return [f["format"]['name'] for f in erc_json["functions"] if f["format"]['view']]
...
def get_anchored_state_variable(f: FunctionContract) -> StateVariable:
    if f is None:    
        return None
    if isinstance(f, StateVariable):
        return f
    # logger.debug(f"get_anchored: {f.name} lines={f.source_mapping.lines[0]}")
    defby_op = {}
    for node in f.nodes:
        for ir in node.irs:
            if hasattr(ir,"lvalue") and ir.lvalue:
                defby_op[ir.lvalue] = ir
            
    for var in f.return_values:
        # logger.debug(f"get_anchored: return={var} type={type(var)}")
        if isinstance(var, StateVariable):
            return var
        elif isinstance(var, ReferenceVariable):
            return var.points_to_origin
        elif isinstance(var, LocalVariable) or isinstance(var, TemporaryVariable):
            if isinstance(var.expression, CallExpression):
                ...
                if not returned_by_arr:
                    # logger.error("cannot find callee to get anchored state variable")
                    return None
                returned_by = returned_by_arr[0]
                return get_anchored_state_variable(returned_by)
            else:
                if var in defby_op:
                    init_op = defby_op[var]
                    if isinstance(init_op, InternalCall):
                        return get_anchored_state_variable(init_op.function)
    # logger.error(f"does not find anchored state variable at {f.name}")
    return None
...
def find_throw_nodes(initf: FunctionContract, params: List[int] = None, msg_sender=False) -> Set[Node]:
    """get matched throw related nodes
...
def get_erc(src, content=None):
    ...
    if ("ERC1155" in content or "erc1155" in content) and "safeBatchTransferFrom" in content and "TransferSingle" in content:
        return "1155"
    elif ("ERC721" in content or "erc721" in content) and "emit ApprovalForAll" in content:
        return "721"
