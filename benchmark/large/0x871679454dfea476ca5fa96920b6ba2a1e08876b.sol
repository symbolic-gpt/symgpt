...
contract Monsterx is IERC721Receiver, ERC721, ReentrancyGuard, Ownable {
    ...
    function purchaseAsset(uint256 _tokenId) external payable {
        require(_exists(_tokenId), "Nonexistent token");
    ...
    function purchaseAssetUnmited(string memory _uri, address seller, RoyaltyDetails memory royalty, PaymentSplit[] memory _paymentSplit) external payable{
        uint256 _tokenId = tokenizeAsset(_uri);
        idToSale[_tokenId] = SaleDetails(
        _tokenId,
        seller,
    ...
    function releaseEscrow(uint256 _tokenId) public {
        require(_exists(_tokenId), "Nonexistent token");
        ...
        IERC721(address(this)).approve(0x0000000000000000000000000000000000000000,_tokenId);
    ...
    function requestCancellation(uint256 _tokenId, string memory description) external{
        require(_exists(_tokenId), "Nonexistent token");
    ...
    function cancelOrder(uint256 _tokenId, string memory description) public{
        require(_exists(_tokenId), "Nonexistent token");
    ...
    function reportDispute(uint256 _tokenId, string memory description) external {
        require(_exists(_tokenId), "Nonexistent token");
    ...
    function placeBidUnminted(string memory _uri, address seller, uint256 price, RoyaltyDetails memory royalty, PaymentSplit[] memory _paymentSplit) external payable {
        uint256 _tokenId = tokenizeAsset(_uri);
        idToSale[_tokenId] = SaleDetails(
        _tokenId,
        seller,
    ...
    function acceptBid(uint256 _tokenId, uint256 _bidId) external {
        require(_exists(_tokenId), "Nonexistent token");
    ...
    function cancelBid(uint256 _bid) external {
    ...
    function updatePrice(uint256 _tokenId, uint256 price) external {
        require(_exists(_tokenId), "Nonexistent token");
        require(idToSale[_tokenId].status == uint256(SaleStatus.Active), "Sale is not live");
    ...
    function setAdmin(address[] memory admins, bool _isAdmin) external onlyOwner {
        for(uint256 i=0;i<admins.length;i++){
            isAdmin[admins[i]] = _isAdmin;
        }
    }

    function setCurators(address[] memory curators, bool _isCurator) external {
        require(isAdmin[msg.sender], "Access Denied");
    ...
    function setEscrowReleaseTime(uint256 _time) external{
        require(isAdmin[msg.sender], "Access Denied");
