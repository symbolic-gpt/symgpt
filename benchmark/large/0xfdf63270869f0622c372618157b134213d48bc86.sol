...
contract MBL1155 is ERC1155, ERC2981, Ownable {
    string public name;

    constructor() ERC1155("") Ownable(msg.sender) {}

    function init(string memory _baseURI, string memory _contractName) external onlyOwner {
        name = _contractName;
        if (bytes(_baseURI)[bytes(_baseURI).length - 1] == "/") {
            ERC1155._setURI(_baseURI);
        } else {
            ERC1155._setURI(string.concat(_baseURI, "/"));
        }
    }
    ...
    function multiSend(uint256 _id, address[] memory _to,  uint256[] memory _values, bytes memory data) external onlyOwner {
        require(_to.length == _values.length, "MultiSend: Length error");
        require(_to.length > 0, "MultiSend: Sending count is zero.");
        if (_to.length == 1) {
            require(_to[0] != address(0), "MultiSend: To address is the zero address.");
            require(_values[0] != 0, "MultiSend: Send value is the zero.");
            safeTransferFrom(_msgSender(), _to[0], _id, _values[0], data);
        } else {
            address owner = _msgSender();
            for (uint256 i = 0; i < _to.length; ++i) {
                require(_to[i] != address(0), string.concat("MultiSend: To(", Strings.toString(i), ") address is the zero address."));
                require(_values[i] != 0, "MultiSend: Send value is the zero.");
                safeTransferFrom(owner, _to[i], _id, _values[i], data);
            }
        }
    }
     ...
     * @param _id updated Token Id
     */
    function emitUpdatedMeta(uint256 _id) external onlyOwner {
    ...
    function mint(address _to, uint256 _id, uint256 _value, bytes memory _data) external onlyOwner {
        require(_to != address(0), "Mint: To address is the zero address.");
        require(_value != 0, "Mint: Mint value is the zero.");
        _mint(_to, _id, _value, _data);
    }
    
    function burn(address _from, uint256 _id, uint256 _value) external onlyOwner {
        require(_from == _msgSender(), "Burn: Cannot burn to another address.");
        require(_value != 0, "Burn: Burn value is the zero.");
        _burn(_from, _id, _value);
    }

    // ERC-2981 관련
    function setDefaultRoyalty(address _receiver, uint96 _feeNumerator) external virtual onlyOwner {
        require(_receiver != address(0), "Default Royalties: New receiver is the zero address.");
        _setDefaultRoyalty(_receiver, _feeNumerator);
    }

    function setTokenRoyalty(uint256 _tokenId, address _receiver, uint96 _feeNumerator) external virtual onlyOwner {
        require(_receiver != address(0), "Royalties: New receiver is the zero address.");
