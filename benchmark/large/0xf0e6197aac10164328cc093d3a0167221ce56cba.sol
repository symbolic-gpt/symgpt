...
contract AdPanel {
    ...
    address public HolderCurrencyToken;
    ...
    function createRentableToken(uint256 _areaSize) public onlyOwner {
        ...
        TokenInfo memory newToken = TokenInfo({
            ...
            rentalInfo: RentalInfo(address(0),"", 0, 72, 0, 0)
        });
    ...
    function setTokenData(uint256 _tokenId, address currencyTokenAddress, string memory _IPFSAddress) public {
        TokenInfo storage rentableToken = rentableTokens[_tokenId];
        require(msg.sender == rentableToken.owner, "Only the owner can change the token properties");
        ...
        rentableToken.IPFSAddress = _IPFSAddress;
    }

    function setTokenIPFSAddress(uint256 _tokenId, string memory _IPFSAddress) public {
        TokenInfo storage rentableToken = rentableTokens[_tokenId];
        require(msg.sender == rentableToken.owner ||msg.sender == rentableToken.rentalInfo.RenterAddress , "Only the owner or the renter can change the token IPFSAddress");
        
        if(msg.sender == rentableToken.rentalInfo.RenterAddress){
            rentableToken.rentalInfo.IPFSAddress = _IPFSAddress;
        }else{
            rentableToken.IPFSAddress = _IPFSAddress;
        }
    ...
    function setRentingAvailability(uint256 _tokenId, bool _available, uint256 _pricePerHour, uint _maxRentalHours) public {
    ...
    function getTokenData(uint256 _tokenId) public view returns (string memory _name, string memory _description, address owner, uint256 AreaSize, uint256 Price, bool availableForRenting, address currencyToken, string memory IPFSAddress, TokenTransform memory tokenTransform, RentalInfo memory rentalInfo) {
        require(_tokenId > 0 && _tokenId <= tokenCount, "Invalid panelId");
        TokenInfo memory rentableToken = rentableTokens[_tokenId];
        return (name, description, rentableToken.owner, rentableToken.AreaSize, rentableToken.holderPrice, rentableToken.availableForRenting, rentableToken.currencyToken, rentableToken.IPFSAddress, rentableToken.tokenTransform, rentableToken.rentalInfo);
    }

     // Define a function to get rentable token data
    function getRentingData(uint256 _tokenId) public view returns (address RenterAddress, string memory IPFSAddress
    , uint256 pricePerHour, uint256 MaxRentalHours, uint256 StartRentTime,uint256 EndRentTime) {
        ...
        if (!isRented(_tokenId)) {
            ...
            tokenData.rentalInfo.IPFSAddress = rentableToken.IPFSAddress;
        } else {
            tokenData.availableForRenting = false;
            
        }

        return (tokenData.rentalInfo.RenterAddress, tokenData.rentalInfo.IPFSAddress
    ...
    function getTokenTransform(uint256 _tokenId) public view returns (int256 locationX, int256 locationY, int256 locationZ, int256 rotationX, int256 rotationY, int256 rotationZ, int256 scaleX, int256 scaleY, int256 scaleZ) {
        require(_tokenId > 0 && _tokenId <= tokenCount, "Invalid panelId");
        TokenInfo memory rentableToken = rentableTokens[_tokenId];
        TokenTransform memory tokenTransform = rentableToken.tokenTransform;
        return (tokenTransform.locationX, tokenTransform.locationY, tokenTransform.locationZ, tokenTransform.rotationX, tokenTransform.rotationY, tokenTransform.rotationZ, tokenTransform.scaleX, tokenTransform.scaleY, tokenTransform.scaleZ);
    }

    function isRented(uint256 _tokenId) public view returns (bool) {
        TokenInfo memory rentableToken = rentableTokens[_tokenId];
        if (rentableToken.rentalInfo.EndRentTime > block.timestamp) {
            return true;
        } else {
            return false;
        }
    ...
    // Function to buy an ad panel
    function buyAdPanel(uint256 _tokenId) public {
        TokenInfo storage adPanel = rentableTokens[_tokenId];
        //require(adPanel.availableForRenting == false, "Ad panel is already available for rent");
        require(adPanel.owner != address(0), "Ad panel does not exist");
        require(msg.sender != adPanel.owner, "You already own this ad panel");
        require(adPanel.holderPrice > 0, "Ad panel not available for selling");
        require(ERC20(adPanel.currencyToken).balanceOf(msg.sender) >= adPanel.holderPrice, "Insufficient balance");
        require(ERC20(adPanel.currencyToken).allowance(msg.sender, address(this)) >= adPanel.holderPrice, "Insufficient allowance");

        ERC20(adPanel.currencyToken).transferFrom(msg.sender, adPanel.owner, adPanel.holderPrice);
        adPanel.IPFSAddress = "";
        adPanel.owner = msg.sender;
    }

    // Function to change ownership of an ad panel
    function changeOwnership(uint256 _tokenId, address _newOwner) public {
        TokenInfo storage adPanel = rentableTokens[_tokenId];
        require(msg.sender == adPanel.owner, "You are not the owner of this ad panel");
        require(_newOwner != address(0), "Invalid new owner address");
        adPanel.IPFSAddress = "";
        adPanel.owner = _newOwner;
    }
}