...
/**
 * @title ERC20Taxable
 * @dev Extension of ERC20 that adds a tax rate percent.
 */
abstract contract ERC20Taxable is ERC20 {
    // the percent rate for taxable mechanism
    ...
    /**
     * @dev Indicates an error setting the tax receiver address.
     * @param taxReceiver New tax receiver address that is wanted to set.
     */
    error ERC20TaxableInvalidTaxReceiver(address taxReceiver);

    /**
     * @dev Sets the value of the `_taxRate` and the `_taxReceiver`, excludes the deployer and the tax receiver.
     */
    constructor(uint256 taxFeePercent_, address taxReceiver_) {
        _setTaxRate(taxFeePercent_);
        _setTaxReceiver(taxReceiver_);
        _setExclusionFromTaxFee(_msgSender(), true);
        _setExclusionFromTaxFee(taxReceiver_, true);
    }

    /**
     * @dev Returns the percent rate for taxable mechanism.
     *
     * For each transfer the percent amount will be calculated and moved to deposit address.
     */
    function taxFeePercent() external view returns (uint256) {
        return _taxRate;
    }
    ...
    function _transferWithFee(address from, address to, uint256 value) internal {
        if (_taxRate > 0 && !(_isExcludedFromTaxFee[from] || _isExcludedFromTaxFee[to])) {
            ...
            if (taxAmount > 0) {
                super._update(from, _taxReceiver, taxAmount);
                unchecked {
                    // Overflow not possible: taxAmount < amount => _taxRate is required to be <= 10.
                    value -= taxAmount;
                }
            }
        }
    ...
    /**
     * @dev Sets the amount of tax fee percent. Max 10%.
     *
     * WARNING: it allows everyone to set the fee. Access controls MUST be defined in derived contracts.
     *
     * @param taxFeePercent_ The amount of tax fee percent.
     */
    function _setTaxRate(uint256 taxFeePercent_) internal virtual {
        if (!(taxFeePercent_ <= 10)) {
            revert ERC20TaxableInvalidTaxFee(taxFeePercent_);
        }

        _taxRate = taxFeePercent_;
    }
     ...
     *
     * @param taxReceiver_ The deposit address for tax.
     */
    function _setTaxReceiver(address taxReceiver_) internal virtual {
        if (taxReceiver_ == address(0)) {
            revert ERC20TaxableInvalidTaxReceiver(taxReceiver_);
        }
...
contract TaxableERC20 is ERC20Detailed, ERC20Taxable, TokenOwner, ServicePayer {
    constructor(
        ...
        ERC20Taxable(taxFeePercent_, taxReceiver_)
     ...
     *
     * @param taxFeePercent The amount of tax fee percent.
     */
    function setTaxFeePercent(uint256 taxFeePercent) external onlyOwner {
        super._setTaxRate(taxFeePercent);
    }

    /**
     * @dev Sets the address where to collect tax fee.
     *
     * NOTE: restricting access to owner only. See `ERC20Taxable::_setTaxReceiver`.
     *
     * @param taxReceiver The deposit address for tax.
     */
    function setTaxReceiver(address taxReceiver) external onlyOwner {
        super._setTaxReceiver(taxReceiver);
    }
