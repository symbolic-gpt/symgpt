    ...
    ERC20("AMERICAN SHIBA", "USHIBA")
{
    ...
    uint256 public MAX_WALLET = MAX_SUPPLY / 5; // Max wallet is 12 Quad at deployment
    uint256 public MAX_TX_LIMIT = MAX_SUPPLY / 80; // Max transaction is 750T at deployment
    ...
    uint256 public swapFee = 300; // Initialize swap fee (acting as feeoperator) to 3%, you can adjust this value as needed
    bool public feesEnabled = true; // Fees are enabled by default
    // no longer used since Version 4.24.0.13 uint256 public swapFeePercent = 100;    //1.0% fee for liquidity, cannot exceed 5%, in basis points.
        ...
        returns (bool)
    {
        ...
        // Apply maximum transaction limit check based on the type of interaction
        if (senderIsContract) {
            // Assuming this is a buy operation since a contract is sending tokens
            require(
                amount <= MAX_TX_LIMIT,
                "Buy transfer amount exceeds the maximum transaction limit"
            );
        } else if (recipientIsContract) {
            // Assuming this might be a sell operation since tokens are going to a contract
            require(
                amount <= MAX_TX_LIMIT,
                "Sell transfer amount exceeds the maximum transaction limit"
            );
        } else {
            // Regular transfer
            require(
        ...
        // Check for max wallet limit unless the recipient is exempt (like swap contracts)
        ...
        uint256 feeAmount = 0;
        if (feesEnabled) {
            bool applySellFee = isSellFeeAddress[finalRecipient] &&
                recipientIsContract;

            // Apply fees based on the type of transaction
            if (
                !isWhitelistedFromFees[_msgSender()] &&
                !isWhitelistedFromFees[finalRecipient]
            ) {
                if (senderIsContract) {
                    // This is a buy transaction (tokens being bought from a contract)
                    feeAmount = (amount * buyFee) / 10000;
                } else if (recipientIsContract) {
                    // This is a swap transaction (tokens being sent to a contract)
                    feeAmount = (amount * swapFee) / 10000;
                    if (applySellFee) {
                        // Additional sell fee for specified contract interactions
