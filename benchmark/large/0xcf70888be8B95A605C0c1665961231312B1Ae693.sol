...
// File: contracts-main/contracts/metatx/ContextStorage.sol
...
// File: contracts-main/contracts/nfts/ERC721VoucherMintableBurnable.sol
...
contract ERC721VoucherMintableBurnable is ContextStorage, ERC721URIStorage, EIP712, AccessControl {
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  uint256 public totalSupply;
  /// @notice Map pointing to used vouchers. Prevents frauds and double usage of single voucher (after burn).
  mapping(bytes32 => bool) public usedVouchers;

  constructor(address minter, string memory name_, string memory symbol_, string memory domain, string memory version)
    ERC721(name_, symbol_)
    EIP712(domain, version) {
      require(minter != address(0), "The initial admin/minter cannot be zero");
  ...
  function redeem(address redeemer, NFTVoucher calldata voucher) external returns (uint256) {
    ...
    address signer = _verify(digest, voucher.signature);
    ...
    require(!usedVouchers[digest], "Voucher has been already used");
    ...
    _safeTransfer(signer, redeemer, voucher.tokenId, bytes(''));
  ...
  function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("NFTVoucher(uint256 tokenId,string uri,uint256 expiration,address redeemer)"),
      voucher.tokenId,
  ...
  /// @param digest hash of a voucher, calculated in advance
  /// @param signature A signature to be verified
  function _verify(bytes32 digest, bytes calldata signature) internal pure returns (address) {
  ...
  /**
   * @dev the owner or approved address can burn an existing token.
   * Token URI is cleared by the underlying contract.
   */
  function burn(uint256 tokenId) external virtual {
    require(_isApprovedOrOwner(_msgSender(), tokenId), "The spender not allowed to burn");
    totalSupply--;
    _burn(tokenId);
  }

  function lockTrustedForwarders() external onlyRole(DEFAULT_ADMIN_ROLE) {
    _lockTrustedForwarders();
  }

  function setTrustedForwarder(address forwarder, bool active) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
// File: contracts-main/contracts/nfts/LoeHero.sol

//SPDX-License-Identifier: MIT
pragma solidity 0.8.9;


contract LoeHero is ERC721VoucherMintableBurnable {
  string private constant _name_ = "Legends of Elysium: Heroes";
  string private constant _symbol_ = "LOE-HEORES";
  string private constant SIGNING_DOMAIN = "LoeHero-Voucher";
  string private constant SIGNATURE_VERSION = "1";
  string public constant baseURI = "https://legendsofelysium.io";

  // solhint-disable-next-line no-empty-blocks
  constructor(address minter)
    ERC721VoucherMintableBurnable(minter, _name_, _symbol_, SIGNING_DOMAIN, SIGNATURE_VERSION) {}
}