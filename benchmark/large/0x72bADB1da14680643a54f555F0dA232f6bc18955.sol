...
contract blueToken is ERC20, Ownable {
    constructor() ERC20("blueToken", "bToken") Ownable(msg.sender) {
    }
...
contract DepositContract {
    ...
    blueToken public bTokens; // Address of the BlueToken contract
    uint256 public feePercentage = 5; // 5% fee
    uint256 public ethToTokenRatio = 1e18; // 1 ETH = 1 ERC20 token (in Wei)
    address public feeWallet; // Address of the wallet to receive fees
    ...
    constructor(address _bTokens, address _feeWallet) {
        contractOwner = msg.sender;
        bTokens = blueToken(_bTokens);
        feeWallet = _feeWallet;
    }

    // Fallback function to accept Ether deposits and mint BTOKEN tokens
    receive() external payable {
        deposit();
    }

    // Function to deposit Ether and mint BTOKEN tokens
    function deposit() public payable {
        require(msg.value > 0, "Ether deposit amount must be greater than 0");

        // Calculate the equivalent BTOKEN amount based on Ether deposited
        uint256 btokensAmount = (msg.value * ethToTokenRatio) / 1 ether;

        // Calculate the fee amount
        uint256 feeAmount = (btokensAmount * feePercentage) / 100;

        // Deduct the fee from the BTOKEN amount
        uint256 amountAfterFee = btokensAmount - feeAmount;

        // Mint BTOKEN tokens to the user
        bTokens.mint(msg.sender, amountAfterFee);
        emit Deposit(msg.sender, msg.value, amountAfterFee);

        // Transfer the fee to the feeWallet
        payable(feeWallet).transfer(feeAmount);

        // Forward the remaining Ether to the contract owner's wallet
        payable(contractOwner).transfer(msg.value - feeAmount);
    }
    ...
    function withdrawERC20(uint256 _amount) external {
        ...
        bTokens.transfer(msg.sender, _amount);
