...
error Aid__DeadlineShouldBeInTheFuture();
error Aid__DonationNotSuccessFully();
error Aid__PleaseSendEnough();
error Aid__NotOwner();
error Aid__DeadlinePassed();
error Aid__TransferFailed();
error Aid__YouCannotWithdrawYet();
error Aid__CampaignDoesNotExist();

contract EtherAid is ERC721URIStorage, ReentrancyGuard {
    ...
    mapping(address => uint256) public addressToNumberOfCampaigns;
    mapping(address => mapping(uint256 => uint256)) investorInvestmentinEachCampaign;

    uint256 public campaignId = 0;
    address public constant mw = 0x1F33F1A303116c35533142F98bb3F43e8e0Dcf85;
    ...
    modifier doesCampaignExist(uint256 _id) {
        ...
        if (campaign.deadline == 0) {
            revert Aid__CampaignDoesNotExist();
        }
        _;
    }

    constructor() ERC721("EtherAid", "AID") {}
    ...
    function createCampaign(
        ...
        if (_deadline <= block.timestamp) {
            revert Aid__DeadlineShouldBeInTheFuture();
        }
        ...
        addressToNumberOfCampaigns[msg.sender]++;
    ...
    function donateToCampaign(
        ...
        payable
        nonReentrant
        isDeadlinePassed(_id)
        doesCampaignExist(_id)
    {
        if (msg.value <= 0) {
            revert Aid__PleaseSendEnough();
        }
        ...
        uint256 fundShares = amount.mul(5).div(100);
        uint256 ownerShares = amount - fundShares;
        (bool success, ) = payable(campaign.owner).call{value: ownerShares}(
            ""
        );
        (bool success1, ) = payable(mw).call{value: fundShares}("");
        if (!success && !success1) {
            revert Aid__TransferFailed();
        }
    }

    function deleteCampaign(
        uint256 _id
    ) external isOwner(_id) nonReentrant doesCampaignExist(_id) {
        Campaign storage campaign = campaigns[_id];
        campaign.owner = address(0);
        campaign.deadline = 0;
        campaign.onMarketplace = false;
        _burn(_id);
        addressToNumberOfCampaigns[msg.sender]--;
    }
    ...
    function getACampaign(uint256 id) public view returns (Campaign memory) {
        Campaign memory item = campaigns[id];
        return item;
    }

    function addressToNumberOfCampaign(
        address user
    ) public view returns (uint256) {
        uint256 noCampaigns = addressToNumberOfCampaigns[user];
        return noCampaigns;
    }
}
