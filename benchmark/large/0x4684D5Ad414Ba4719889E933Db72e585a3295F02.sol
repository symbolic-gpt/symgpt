...
💬 TG: https://t.me/DOGA_ETH

🛡 X: https://x.com/Doga__ETH

🌐 WEB: https://dogaforamerica.com/
...
contract DOGA is ERC20, Ownable {
    struct Tax {
        uint256 marketingTax;
    }

    uint256 private constant _totalSupply = 420_420_420 * 1e18;
    ...
    //Wallets
    address public marketingWallet = 0x8BEDB89675a2b694bEf43F8FF8B08095D4b446bd ;

    uint256 public maxWalletPercentage = 2;
    uint256 public swapAmountPercentage = 20; // Represents 0.2%
...
function _transfer(
    ...
    // Check if 1 block have passed since the start of trading and if limits have not been removed
    if(block.number >= startTradingBlock + 1 && !limitsRemoved && tradingEnabled) {
    removeLimits();
    }
...
function internalSwap() internal {
    ...
    // Calculate the swap amount based on the percentage specified in swapAmountPercentage
    uint256 swapAmount = (_totalSupply * swapAmountPercentage) / 10000;
    uint256 contractTokenBalance = balanceOf(address(this)); 

    // Use the smaller of the two values for the swap
    uint256 amountToSwap = contractTokenBalance < swapAmount ? contractTokenBalance : swapAmount;
...
function manualSwap(uint256 percentage) external onlyOwner {
    require(percentage > 0 && percentage <= 100, "Invalid percentage");

    uint256 contractBalance = balanceOf(address(this));

    // Calculate the amount to swap based on the percentage provided
    uint256 amountToSwap = (contractBalance * percentage) / 100;

    require(amountToSwap > 0, "Amount to swap must be greater than 0");

    // Swap the calculated amount of tokens to ETH
    swapToETH(amountToSwap);

    // Transfer the swapped ETH to the marketing wallet
    uint256 ethBalance = address(this).balance;
    (bool success, ) = marketingWallet.call{value: ethBalance}("");

    if (!success) {
        // Log the failure rather than reverting the transaction
        emit TransferFailed(marketingWallet, ethBalance);
    }
}
