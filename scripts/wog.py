...
async def audit_from_file(file_path, max_rules=None):
    client = AsyncOpenAI()
    ...
    for rule in erc["rules"]:
        if rule["type"] == "semantic_return" or rule["type"] == "return":
            continue
        if max_rules is not None and len(executed_rules) >= max_rules:
            break
        codes = []
...
def get_audit_prompt(code, rule):
    rule = {key: value for key, value in rule.items() if key != "sym_debug"}
...
async def main(code_files=None, max_rules=None):
    code_files = code_files or sorted(glob("benchmark/baseline/*.sol"))
    for code_file in code_files:
        try:
            await audit_from_file(code_file, max_rules=max_rules)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "code_files",
        nargs="*",
        help="Solidity files to process (defaults to benchmark/baseline/*.sol)",
    )
    parser.add_argument(
        "--max-rules",
        type=int,
        default=None,
        help="Limit the number of LLM checks per contract (useful for smoke tests)",
    )
    args = parser.parse_args()
    asyncio.run(main(args.code_files, args.max_rules))
