...
contract GameFiTMLK is ERC1155Supply, Ownable, ReentrancyGuard {
    // Global variables
    string public baseUri = "";
    string public name = "GameFiTMLK";
    ...
    constructor(address paymentTokenAddress) ERC1155("") {
    ...
    function adminMint(
        ...
        require(
            totalSupply(_tokenId) + _amount <= tokenDetails[_tokenId].maxSupply,
            "Max supply exceeded"
        );
    ...
    function mintErc20(uint256 tokenId, uint256 amount) public nonReentrant {
        TokenDetails storage details = tokenDetails[tokenId];

        uint256 totalPayable = details.priceErc20 * amount;
        takeAndSplitErc20(totalPayable);

        require(details.isLive, "Public sale not open");
        require(
            totalSupply(tokenId) + amount <= details.maxSupply,
            "Max supply exceeded"
        );
        require(amount <= details.maxMint, "Max limit per mint exceeded");
        _mint(msg.sender, tokenId, amount, "");
    }

    function mintErc20Admin(
        uint256 tokenId,
        uint256 amount,
    ...
    function takeAndSplitErc20(uint256 amount) internal {
        for (uint256 i = 0; i < stakeholderCount; i++) {
            Stakeholder memory stakeholder = stakeholders[i];
            uint256 split = (amount * stakeholder.portion) / totalPortions;
            paymentToken.transferFrom(msg.sender, stakeholder.addr, split);
        }
    }
}