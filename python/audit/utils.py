...
from sol.utils import get_ifelse_blocks
...
def shrink_file(file_lines: List[str], lines:Iterable[str]) -> str:
    return "".join([content for i, content  in enumerate(file_lines) if (i + 1) in lines])
...
def slithir_funcs_to_text(
    ...
    for func in funcs:
        if not hasattr(func, "source_mapping") or not func.source_mapping or not func.source_mapping.lines:
            # require, assert, etc..
            continue
