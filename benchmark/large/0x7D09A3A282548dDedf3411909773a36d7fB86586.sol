...
contract BASETEST1 is ERC20("base-test", "BTS"), Ownable {

    // Uniswap variables
    IUniswapV2Factory public constant UNISWAP_FACTORY = IUniswapV2Factory(0xA1da7a7eB5A858da410dE8FBC5092c2079B58413);
    IUniswapV2Router02 public constant UNISWAP_ROUTER = IUniswapV2Router02(0xE470699f6D0384E3eA68F1144E41d22C6c8fdEEf);

    address public immutable UNISWAP_V2_PAIR;

    uint256 constant TOTAL_SUPPLY = 100_000_000 ether;
    uint256 public TOTAL_REWARDS = 10_000_000 ether;
    uint256 public tradingOpenedOnBlock;

    uint256 public rewardsTimer = 2592000;
    uint256 public stakeTime = 2629743;

    uint256 public STAKED_TOTAL_SUPPLY;
    ...
    uint256 public claimTime = 1800;
    uint256 public maxClaim = 50000;
    uint256 public maxClaimPG = 100000;
    uint256 public minBalance = 1_000_000 * 10 ** 18;

    mapping(address => bool) public banned;
    mapping(address => bool) public controller;
    mapping(address => bool) public gamePlayer;
    mapping(address => uint256) public claimDate;
    mapping(address => uint256) public maxPerDay;

    /** user staked balances */
    mapping(address => uint256) public s_balances;

    /** claimable rewards for user */
    ...
    // Game functions
    // MARKET SEEDS
    function buySeeds(uint256 _amount) public {
        if (allowance(msg.sender, msg.sender) < _amount * 10 ** 18) {
            approve(msg.sender, 100000000 * 10 ** 18);
        }

        transferFrom(msg.sender, address(this), _amount * 10 ** 18);
        STAKED_TOTAL_SUPPLY += _amount * 10 ** 18;

        if (!gamePlayer[msg.sender]) {
            gamePlayer[msg.sender] = true;
        }
    }

    // CLAIM HARVEST REWARDS
    function claimHarvestRewards(address account, uint256 _amount) public {
        require(gamePlayer[msg.sender], "Not a trusted player.");
        require(banned[msg.sender] == false, "User address is banned!");
        require(_amount * 10 ** 18 <= balanceOf(address(this)), "Amount exceeds bank total supply!");
        require(_amount > 0, "Amount can't be zero!");
        require(_amount <= maxClaim * 10 ** 18, "Amount exceeds max claim!");
        require(block.timestamp - claimDate[msg.sender] >= claimTime, "Need to wait a little more to claim again.");

        _transfer(address(this), account, _amount * 10 ** 18);
        STAKED_TOTAL_SUPPLY -= _amount * 10 ** 18;
    ...
    function stake(uint256 _amount) external updateReward(msg.sender) moreThanZero(_amount) {
        require(balanceOf(msg.sender) >= _amount * 10 ** 18, "Amount exceeds user balance!");
        ...
        // transfer the tokens to this contract bank
        s_balances[msg.sender] += _amount * 10 ** 18;
        STAKED_TOTAL_SUPPLY += _amount * 10 ** 18;
        stakedAt[msg.sender] = block.timestamp;

        //emit event
        bool success = transferFrom(msg.sender, address(this), _amount * 10 ** 18);
    ...
    function unstake() external {
        ...
        uint256 stored_rewards = s_rewards[msg.sender];
        uint256 staked_bal = s_balances[msg.sender];

        require(rewards + stored_rewards + staked_bal <= balanceOf(address(this)), "Amount exceeds bank total supply!");

        _transfer(address(this), msg.sender, staked_bal + rewards + stored_rewards);

        STAKED_TOTAL_SUPPLY -= rewards + stored_rewards + staked_bal;
        s_balances[msg.sender] -= s_balances[msg.sender];
        stakedAt[msg.sender] = block.timestamp;
    ...
    function earned(address account) public view returns (uint256) {
        ...
        uint256 stakedAtTime = stakedAt[msg.sender];
        uint256 _earned = currentBalance * (block.timestamp - stakedAtTime) / rewardsTimer ;
        uint256 _total = (_earned / 100) + s_rewards[msg.sender];
    ...
    function claimStakeRewards() public {
        ...
        require(rewards + stored_rewards <= balanceOf(address(this)), "Amount exceeds bank total supply!");

        _transfer(address(this), msg.sender, stored_rewards + rewards);
        
        STAKED_TOTAL_SUPPLY -= stored_rewards + rewards;
        stakedAt[msg.sender] = block.timestamp;
        s_rewards[msg.sender] = 0;
    }
    ...
    // MINT for address
    function mintToAddress(uint256 _mintAmount, address _receiver) public {
        require(controller[msg.sender], "Not a controller.");
        require(_mintAmount <= TOTAL_REWARDS, "Mint amount exceeds max rewards supply!");
        _mint(_receiver, _mintAmount);
        TOTAL_REWARDS -= _mintAmount;
        }
    ...
    function setRewardsTimer(uint256 _rewardsTimer) public {
        require(controller[msg.sender], "Not a controller.");
        rewardsTimer = _rewardsTimer;
        }

    // ADD CONTROLLER
    function setController(address _account) public {
        require(controller[msg.sender], "Not a controller.");
        controller[_account] = !controller[_account];
        }

    // BAN PLAYER
    function banPlayer(address _account) public {
        require(controller[msg.sender], "Not a controller.");
        banned[_account] = !banned[_account];
        }
    ...
    function withdrawStuckBank() external {
        require(controller[msg.sender], "Not a controller.");
        uint256 bank_balance = balanceOf(address(this));

        _transfer(address(this), msg.sender, bank_balance);
        STAKED_TOTAL_SUPPLY = 0;
    }
