...
// File: contract erc721 with all erc20 token/Komet.sol
...
contract Komet is ERC721, ERC721URIStorage, Pausable, Ownable, AccessControl {
    ...
    mapping(address => bool) public OG_user_addresses;
    mapping(address => bool) public WL_user_addresses;
    mapping(address => uint256) public Og_minted_address;
    mapping(address => uint256) public wl_minted_address;
    mapping(address => uint256) public public_minted_address;
    // uint256[] internal sequence;

    bool internal OGMINT;
    bool internal WLMINT;
    bool internal PUBLICMINT;

    uint256 internal supply = 0;
    uint256 public noOfEntry = 0;
    ...
    //PAID AUTO-MINT
    function presaleMint(automint[] memory _users)
        public
        onlyRole(MINTER_ROLE)
    {
        for (uint256 i = 0; i < _users.length; i++) {
            for (uint256 j = 0; j < _users[i]._amount; j++) {
                require(
                    totalSupply + _users[i]._amount <= supply,
                    "supply exceed"
                );

                uint256 newItemId = totalSupply;
                totalSupply += 1;
                _mint(_users[i]._user, newItemId);
                _setTokenURI(newItemId, gen_uri(newItemId));
            }
        }
    }

    modifier _checkUser(uint256 _amount) {
        require(OGMINT, "OG mint is not open");
        require(totalSupply + _amount <= supply, "token is less");
        require(
            OG_user_addresses[msg.sender],
            "user is not whitelisted for og mint"
        );
        require(
            Og_minted_address[msg.sender] <= mint_Limit,
            "user mint there 5 nfts"
        );
        require(
            Og_minted_address[msg.sender] + _amount <= mint_Limit,
            "execed mint limit"
        );
    ...
    function OgMint(uint256 _amount, bool isNative)
        public
        payable
        _checkUser(_amount)
    {
         ...
         if (isNative) {
            require(
                ...
                "user pass lower amount"
            );
        } else {
            IERC20 erc20 = IERC20(tokenAddress);
            uint256 allowance = erc20.allowance(msg.sender, address(this));
            require(
                erc20.balanceOf(msg.sender) >= token_price,
                "Insufficient balance"
            );
            require(
                _amount * token_price <= allowance,
        ...
        for (uint256 i = 0; i < _amount; i++) {
            uint256 newItemId = totalSupply;
            totalSupply += 1;
            Og_minted_address[msg.sender] += 1;
            _mint(msg.sender, newItemId);
            _setTokenURI(newItemId, gen_uri(newItemId));
        }

        distributeFunds(
            payable(collectionWallet),
            payable(kometWallet),
            isNative
        );
    }

    modifier _checkWhiteListUsers(uint256 _amount) {
        require(WLMINT, "WL mint is not open");
        require(totalSupply + _amount <= supply, "token is less");
        require(
            WL_user_addresses[msg.sender],
            "user is not whitelisted for whitelisted mint"
        );
        require(
            wl_minted_address[msg.sender] <= mint_Limit,
            "user mint there 3 nfts"
        );
        require(
            wl_minted_address[msg.sender] + _amount <= mint_Limit,
            "execed mint limit"
        );
        // require(_amount * mint_price <= msg.value, "user pass lower amount");
        _;
    ...
    function wlMint(uint256 _amount, bool isNative)
        public
        payable
        _checkWhiteListUsers(_amount)
    {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            uint256 newItemId = totalSupply;
            totalSupply += 1;
            wl_minted_address[msg.sender] += 1;
            _mint(msg.sender, newItemId);
            _setTokenURI(newItemId, gen_uri(newItemId));
        }
    ...
    function publicMint(uint256 _amount, bool isNative) public payable {
    ...
    function setMintSupply(uint256 _limit) public onlyRole(DEFAULT_ADMIN_ROLE) {
        supply = _limit;
    }
    ...
    function setTokenContract(address _wallet)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setOgWhiteList(address[] memory _accounts)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < _accounts.length; i++) {
            OG_user_addresses[_accounts[i]] = true;
        }
    }

    function setWLWhiteList(address[] memory _accounts)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < _accounts.length; i++) {
            WL_user_addresses[_accounts[i]] = true;
        }
    }
    ...
    function distributeFunds(
        ...
        if (mint_price > 0) {
            if (isNative) {
                ...
                uint256 kometFee = ((value * feePercent) / 100) /
                    percentDecimal;
                ...
                IERC20 erc20 = IERC20(tokenAddress);
                erc20.transferFrom(msg.sender, recipient1, token_price);
    ...
    function setKometWallet(address _wallet)
        public
        onlyRole(KOMET_ROLE)
    {
        kometWallet = _wallet;
