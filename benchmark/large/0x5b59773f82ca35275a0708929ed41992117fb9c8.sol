...
//  Begin Constructor
    ...
    constructor(address routerAddress, address _feeCollector, address _newGeneralTreasury) {
...
function isContract(address account) internal view returns (bool) {
    ...
    // Allows the owner to update the Fee Collector Wallet Address
...
function transfer(address recipient, uint256 amount) public override returns (bool) {
    ...
    _transfer(_msgSender(), feeCollector, feeAmount);
...
function SwapUSHIBAForNative(uint256 tokenAmount) public {
    ...
    payable(feeCollector).transfer(swapFee);
