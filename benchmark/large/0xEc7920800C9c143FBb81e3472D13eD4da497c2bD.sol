 ...
 */
contract ERC1155Token is Controllable, ERC1155Supply, IERC2981 {
     ...
     * @dev The `account` is not authorized.
     */
    error ERC1155TokenUnauthorizedAccount(address account);

    /**
     * @dev Token identified by `identifier` already exists.
     */
    error ERC1155TokenIdentifierAlreadyExists(uint256 identifier);

    /**
     * @dev Token identified by `identifier` does not exist.
     */
    error ERC1155TokenIdentifierDoesNotExist(uint256 identifier);

    /**
     * @dev Value is the same as current one.
     */
    error ERC1155TokenTheSameValue();

    /**
     * @dev List cannot be empty.
     */
    error ERC1155TokenEmptyList();

    /**
     * @dev Lists must have the same length.
     */
    error ERC1155TokenListsLengthMismatch();

    /**
     * @dev Emitted when the metadata `uriPrefix` is set to a new URI.
     */
    event MetadataUriChanged(string indexed uriPrefix);

    /**
     * @dev Emitted when the `checksum` of token identified by `identifier` is set to a new value.
     */
    event MetadataChecksumChanged(uint256 indexed identifier, uint256 indexed checksum);

    /**
     * @dev Emitted when the `checksums` of tokens identified by `identifiers` are set to new values.
     */
    event MetadataChecksumBatchChanged(uint256[] indexed identifiers, uint256[] indexed checksums);

    /**
     * @dev Emitted when the `minter` address changes.
     */
    ...
    /**
     * @dev Throws if called by any account other than the controller or the minter.
     */
    modifier onlyControllerOrMinter() {
        if (msg.sender != controller && msg.sender != minter) {
            revert ERC1155TokenUnauthorizedAccount(msg.sender);
        }
        _;
    }
    ...
    modifier onlyExistingToken(uint256 identifier) {
        if (!exists(identifier)) {
            revert ERC1155TokenIdentifierDoesNotExist(identifier);
        }
        _;
    }

    /**
     * @dev Allows to mint non-fungible token identified by `identifier` and `checksum` by the controller or the owner and assign it to `beneficiary`.
     */
    function mintNonFungible(uint256 identifier, uint256 checksum, address beneficiary) external onlyControllerOrMinter {
        if (exists(identifier)) {
            revert ERC1155TokenIdentifierAlreadyExists(identifier);
        }
    ...
    /**
     * @dev Allows to mint multiple non-fungible tokens identified by `identifiers` and `checksums` by the controller or the owner and assign them to `beneficiary`.
     */
    function mintNonFungibleBatch(uint256[] calldata identifiers, uint256[] calldata checksums, address beneficiary) external onlyControllerOrMinter {
        uint256[] memory values = new uint256[](identifiers.length);

        for (uint256 i = 0; i < identifiers.length; ++i) {
            if (exists(identifiers[i])) {
                revert ERC1155TokenIdentifierAlreadyExists(identifiers[i]);
            }
    ...
    function setMetadataUriPrefix(string calldata uriPrefix) external onlyOwner {
        if (keccak256(abi.encodePacked(_uriPrefix)) == keccak256(abi.encodePacked(uriPrefix))) {
            revert ERC1155TokenTheSameValue();
        }
     ...
     */
    function setMetadataChecksum(uint256 identifier, uint256 checksum) external onlyMinter onlyExistingToken(identifier) {
        if (_checksums[identifier] == checksum) {
            revert ERC1155TokenTheSameValue();
        }
     ...
     */
    function setMetadataChecksumBatch(uint256[] calldata identifiers, uint256[] calldata checksums) external onlyMinter {
        if (identifiers.length == 0 || checksums.length == 0) {
            revert ERC1155TokenEmptyList();
        }

        if (identifiers.length != checksums.length) {
            revert ERC1155TokenListsLengthMismatch();
        }

        for (uint256 i = 0; i < identifiers.length; ++i) {
            if (!exists(identifiers[i])) {
                revert ERC1155TokenIdentifierDoesNotExist(identifiers[i]);
            }

            if (_checksums[identifiers[i]] == checksums[i]) {
                revert ERC1155TokenTheSameValue();
            }
    ...
    /**
     * @dev Sets royalty info (`beneficiary` and `percent`) for token identified by `identifier`.
     */
    function setTokenRoyaltyInfo(uint256 identifier, address beneficiary, uint8 percent) external onlyControllerOrMinter onlyExistingToken(identifier) {
        if (beneficiary == address(0)) {
            delete _tokenRoyalties[identifier];
        } else {
            _tokenRoyalties[identifier] = Royalty(beneficiary, percent, true);
        }
    }

    /**
     * @dev Sets minter address to `newMinter`.
     */
    ...
    function uri(uint256 identifier) public view override onlyExistingToken(identifier) returns (string memory) {
        return string(abi.encodePacked(_uriPrefix, uint160(address(this)).toHexString(), "/", identifier.toString(), ".json"));
    }
    ...
    /**
     * @dev Returns beneficiary and percent for given `royalty`.
     */
    function _royalty(Royalty storage royalty) internal view returns (address, uint8) {
        return (royalty.beneficiary, royalty.percent);
    }

    /**
     * @dev Returns beneficiary and percent for token identified by `identifier` if this token has dedicated settings.
     */
    function tokenRoyalty(uint256 identifier) external view onlyExistingToken(identifier) returns (address, uint8) {
        return _royalty(_tokenRoyalties[identifier]);
    }

    /**
     * @dev See {IERC2981-royaltyInfo}.
     */
    function royaltyInfo(uint256 identifier, uint256 salePrice) public view override onlyExistingToken(identifier) returns (address, uint256) {
        Royalty memory royalty = _tokenRoyalties[identifier];
        ...
        if (salePrice == 0) {
            return (royalty.beneficiary, 0);
        }

        return (royalty.beneficiary, salePrice.mulDiv(royalty.percent, 100));
    }

    /**
     * @dev Returns TRUE if token identified by `identifier` has own royalty settings.
     */
    function hasTokenRoyalty(uint256 identifier) external view onlyExistingToken(identifier) returns (bool) {
        return _tokenRoyalties[identifier].isSet;
    }

    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceIdentifier) public view override(ERC1155, IERC165) returns (bool) {
...
contract ERC1155TokenController is Controller {
    ...
    /**
     * @dev The `token` is invalid (zero address or not a contract).
     */
    error ERC1155TokenControllerInvalidToken(address token);

    /**
     * @dev List cannot be empty.
     */
    error ERC1155TokenControllerEmptyList();

    /**
     * @dev Lists must have the same length.
     */
    error ERC1155TokenControllerListsLengthMismatch();

    /**
     * @dev Emitted when the token identified by the `identifier` is minted for the `beneficiary`.
     */
    event Mint(uint256 indexed identifier, address indexed beneficiary);

    /**
     * @dev Emitted when tokens identified by `identifiers` are minted for the `beneficiary`.
     */
    event MintBatch(uint256[] indexed identifier, address indexed beneficiary);

    /**
     * @dev Creates token controller instance with token set to `token` and signer set to `signer`.
     */
    constructor(address token, address signer) Controller(signer) {
        if (token == address(0) || token.code.length == 0) {
            revert ERC1155TokenControllerInvalidToken(token);
        }
    ...
    /**
     * @dev Allows to mint non-fungible token identified by `identifier` and `checksum` by bearer of valid `signature` and assign it to `beneficiary`.
     */
    function mintNonFungible(uint256 identifier, uint256 checksum, address beneficiary, uint256 timestamp, bytes calldata signature) external onlyValidSignatureAndTimestamp(signature, timestamp) {
        _markSignatureAsUsed(signature);

        ERC1155Token(_token).mintNonFungible(identifier, checksum, beneficiary);

        emit Mint(identifier, beneficiary);
    }

    /**
     * @dev Allows to mint multiple non-fungible tokens identified by `identifiers` by bearer of valid `signature` and assign them to `beneficiary`.
     */
    function mintNonFungibleBatch(uint256[] calldata identifiers, uint256[] calldata checksums, address beneficiary, uint256 timestamp, bytes calldata signature) external onlyValidSignatureAndTimestamp(signature, timestamp) {
        if (identifiers.length == 0 || checksums.length == 0) {
            revert ERC1155TokenControllerEmptyList();
        }

        if (identifiers.length != checksums.length) {
            revert ERC1155TokenControllerListsLengthMismatch();
        }

        _markSignatureAsUsed(signature);

        ERC1155Token(_token).mintNonFungibleBatch(identifiers, checksums, beneficiary);

        emit MintBatch(identifiers, beneficiary);
    }

    /**
     * @dev Sets royalty info (`beneficiary` and `percent`) for token identified by `identifier` by bearer of valid `signature`.
     */
    function setTokenRoyaltyInfo(uint256 identifier, address beneficiary, uint8 percent, uint256 timestamp, bytes calldata signature) external onlyValidSignatureAndTimestamp(signature, timestamp) {
        _markSignatureAsUsed(signature);

        ERC1155Token(_token).setTokenRoyaltyInfo(identifier, beneficiary, percent);
    }
}