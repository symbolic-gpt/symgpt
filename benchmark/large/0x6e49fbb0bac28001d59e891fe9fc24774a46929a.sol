...
contract PiratePigzOnPolygon is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[4500] public remainingIds;
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      // Split payment
      uint256 mintShareM = _amount.div(mintReward); //16% Dev
      uint256 mintShareS = _amount.div(mintReward); //16% Dev
      uint256 mintShareR = _amount.div(mintSplit); //0% reflections
      uint256 OwnerShare = _amount.sub(mintShareM + mintShareS + mintShareR);
      // Save dividend for community
      reflectDividend(mintShareR); //0%
      // Send owner/dev shares
      payable(owner()).transfer(OwnerShare);
      payable(0x39e8fD222c39E37b8A157Bd9AAF1084733061902).transfer(mintShareS);
      payable(0x39e8fD222c39E37b8A157Bd9AAF1084733061902).transfer(mintShareM);
